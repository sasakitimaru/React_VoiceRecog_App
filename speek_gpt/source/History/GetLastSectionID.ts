...
function getMaxSectionID(conversations) {
    const sectionIDs = conversations.map((conversation) => parseInt(conversation.sectionID, 10));
    const maxSectionID = Math.max(...sectionIDs);
    console.log('maxSectionID:', maxSectionID)
    return maxSectionID;
}  

async function fetchLastSectionID(userId: String): Promise<String | null> {
    try {
        ...
        console.log('sortedLastID:', lastSectionID.toString());
        return lastSectionID.toString();
    }catch (error) {
